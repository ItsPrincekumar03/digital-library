...
function forbidden(message) {
    const err = new Error(message || 'You do not have permission to modify this book.');
    err.status = 403;
    return err;
}
...
async function createBook(ownerUserId, { title, description, coverPath, publicationDate }) {
    const bookId = await bookRepository.create({ ownerUserId, title, description, coverPath, publicationDate });
    ...
    if (status !== undefined) {
        if (!isAdmin) {
            if (!USER_ALLOWED_STATUSES.includes(status)) {
                throw forbidden('Only an admin can approve, reject, publish, or archive a book.');
            }
        }
...
module.exports = { createBook, getAllBooks, getBookById, updateBook, archiveBook };